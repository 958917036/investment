...
def main():
    ...
    log_start("breakout_screener", "run", f"pool={pool}")
    result = run_l1("by_strategy", {"strategy": "breakout", "pool": pool})
    print(f"breakout: {result['stock_count']} stocks, {result['duration_ms']}ms")
    log_end("breakout_screener", "run", f"{result['stock_count']} stocks")
    return result
