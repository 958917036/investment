...
def main():
    ...
    log_start("garp_screener", "run", f"pool={pool}")
    result = run_l1("by_strategy", {"strategy": "garp", "pool": pool})
    print(f"garp: {result['stock_count']} stocks, {result['duration_ms']}ms")
    log_end("garp_screener", "run", f"{result['stock_count']} stocks")
    return result
