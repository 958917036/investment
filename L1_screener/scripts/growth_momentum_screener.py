#!/usr/bin/env python3
"""
Graham Growth Momentum Screener (L1策略)
原Graham策略→实战升级为Growth Momentum
筛选核心：营收增速≥20% + 动量确认 + EPS增长
"""
...
def get_all(scope="all"):
    ...
    if scope == "full":
        ...
        info("growth_momentum", f"粗筛后保留: {len(df)} 只（价>0, 市值≥5亿, 日均成交≥500万）")
    return df
...
def score_graham(q):
    ...
    # 成长分: 用E/P收益率的快速增长潜力近似 + 市值合理
    growth_score = 0.0
    if pe > 0:
        ep = 1.0 / pe
        # 高增长公司PE相对高但E/P>2%还算可接受
        if pe <= 30:
            growth_score += 0.30
    ...
    # 使用配置中的阈值
    if total >= thresholds[2]:  # 0.55
        signal = 1.0
    elif total >= thresholds[1]:  # 0.40
        signal = 0.6
    elif total >= thresholds[0]:  # 0.25
        signal = 0.3
    else:
        signal = 0.0
...
def main(pool='index800'):
    ...
    log_start("growth_momentum", "run_screener", f"pool={pool}")
    info("growth_momentum", f"股票池: {'全市场(粗筛)' if pool == 'full' else '沪深300+中证500'}")

    frozen_codes, observing = load_freezes()
    info("growth_momentum", f"冷冻: {len(frozen_codes)}, 观察: {len(observing)}")

    if pool == 'full':
        info("growth_momentum", "获取全市场股票列表(粗筛过滤)...")
        all_stocks = get_all("full")
        info("growth_momentum", f"全市场粗筛后: {len(all_stocks)} 只")
    else:
        stocks_hs = get_all("hs300")
        stocks_zz = get_all("zz500")
        import pandas as pd
        all_stocks = pd.concat([stocks_hs, stocks_zz]).drop_duplicates(subset=['code'])
        info("growth_momentum", f"沪深300+中证500: {len(all_stocks)}")

    codes = [f"{'sh' if c.startswith(('6','8')) else 'sz'}{c}" for c in all_stocks['code'].tolist()]
    quotes = batch_fetch_quotes(codes)
    info("growth_momentum", f"行情获取: {len(quotes)}")

    candidates = [q for code,q in quotes.items() if code not in frozen_codes and pre_filter(q)[0]]
    info("growth_momentum", f"前置通过: {len(candidates)}")
    ...
    for q in candidates:
        ...
        if s['signal'] >= min_signal:
            results.append({
                ...
                'price': q['price'], 'pe': q['pe'], 'pb': q['pb'],
                'mcap_100m': q['mcap_100m'],
                'signal': s['signal'], 'score': s['score'],
                'breakdown': s['breakdown'], 'reasons': s['reasons'],
                'strategy': STRATEGY_NAME, 'timestamp': datetime.now().isoformat()
            })
    ...
    info("growth_momentum", f"\n通过: {len(results)}, Top10:")
    for r in top[:10]:
        print(f"  {r['code']} {r['name']:8s} | 价:{r['price']:>8.2f} | PE:{r['pe']:>6.1f} | 信号:{r['signal']:.1f}")

    output = {
        'strategy': STRATEGY_NAME, 'timestamp': datetime.now().isoformat(),
        'total_stocks': len(all_stocks), 'frozen': len(frozen_codes),
        'after_prefilter': len(candidates), 'passed': len(results), 'top200': top
    }
    outfile = os.path.join(RECORDS_DIR, f"{STRATEGY_NAME}_{datetime.now().strftime('%Y%m%d')}.json")
    with open(outfile, 'w') as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    log_end("growth_momentum", "run_screener", f"saved={outfile}")
    return output
