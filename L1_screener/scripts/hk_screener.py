...
def main():
    ...
    log_start("hk_screener", "run", f"pool={pool}")
    result = run_l1("by_strategy", {"strategy": "all", "pool": pool, "market": "hk"})
    print(f"hk_screener: {result['stock_count']} stocks, {result['duration_ms']}ms")
    log_end("hk_screener", "run", f"{result['stock_count']} stocks")
    return result
