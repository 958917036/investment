#!/usr/bin/env python3
"""
Mean Reversion Pullback Recovery Screener (L1策略)
筛选核心：超卖(RSI<45/布林下轨) + 上升趋势未破 + 催化剂
"""
...
def get_all(scope="all"):
    ...
    if scope == "full":
        info("pullback", f"全市场基础列表: {len(df)} 只（已排除ST），正在粗筛...")
        codes = [f"{'sh' if c.startswith(('6','8')) else 'sz'}{c}" for c in df['code'].tolist()]
        quotes = fetch_quotes(codes)
        info("pullback", f"腾讯行情获取: {len(quotes)} 只")
        filtered_codes = []
        ...
        info("pullback", f"粗筛后保留: {len(df)} 只（价>0, 市值≥5亿, 日均成交≥500万）")
    return df
...
def pre(q):
    ...
    if q['pe'] > 100 and q['pe'] > 0: return False  # 超亏的不做
    return True

def score_reversion(q):
    """
    均值回归评分：近1日回调/下跌 + 量不大 + PE合理
    """
    # 从配置读取阈值
    cfg = get_l1_config()
...
def main(pool='index800'):
    ...
    log_start("pullback", "run_screener", f"pool={pool}")
    info("pullback", f"股票池: {'全市场(粗筛)' if pool == 'full' else '沪深300+中证500'}")

    frozen, _ = load_freezes()

    if pool == 'full':
        info("pullback", "获取全市场股票列表(粗筛过滤)...")
        all_df = get_all("full")
        codes = [f"{'sh' if c.startswith(('6','8')) else 'sz'}{c}" for c in all_df['code'].tolist()]
        info("pullback", f"全市场粗筛后: {len(codes)} 只")
    else:
        hs = get_all("hs300")
        zz = get_all("zz500")
        import pandas as pd
        ...
        info("pullback", f"沪深300+中证500: {len(codes)}")
    quotes = fetch_quotes(codes)
    info("pullback", f"行情:{len(quotes)}")

    cands = [q for c,q in quotes.items() if c not in frozen and pre(q)]
    info("pullback", f"前置通过:{len(cands)}")
    ...
    for q in cands:
        ...
        if s['signal'] >= min_signal:
            results.append({
                ...
                'mcap_100m': q['mcap'],'pe': q['pe'],'pb': q['pb'],
    ...
    log_end("pullback", "run_screener", f"saved={f}")
