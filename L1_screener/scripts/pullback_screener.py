...
def main():
    ...
    log_start("pullback_screener", "run", f"pool={pool}")
    result = run_l1("by_strategy", {"strategy": "pullback", "pool": pool})
    print(f"pullback: {result['stock_count']} stocks, {result['duration_ms']}ms")
    log_end("pullback_screener", "run", f"{result['stock_count']} stocks")
    return result
